...
The patch is the **complete** upstream diff `4ee4ae88..1835b7cf` restricted to everything outside the shipped packages: the only excluded paths are the three package directories themselves, so a host at `4ee4ae88` plus this patch becomes `1835b7cf` minus the trio. Nothing outside the packages is dropped. The 66-file diff covers:
...
- Documentation that changed with the feature: root `README*`, `THIRD_PARTY_NOTICES.md`, `docs/`, and `.agents/notes/implemented/architecture/`.
...
A host already at `1835b7cf` or later already contains this wiring and needs nothing. Regenerate the patch from upstream when the fabric host integration changes again:

```sh
git -C <deepseek-harness> diff 4ee4ae888fb8e5c2fb2f81ebd7064d02034e3792 1835b7cf -- . \
