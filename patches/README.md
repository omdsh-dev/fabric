...
- `package.json` + `scripts/dsh-register.mjs` — the `dsh` script preloads a tsx register that pins `tsconfig.base.json`, so tsx resolves the `@deepseek-ai/cordis` paths mapping from the base tsconfig (tsx does not follow `extends` when resolving paths, and the vendored cordis build does not emit `FiberState`). A shell env prefix (`TSX_TSCONFIG_PATH=...`) would work but breaks on Windows.
