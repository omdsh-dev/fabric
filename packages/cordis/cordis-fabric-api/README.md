...
- **Compat API.** A patch-backed gap adapter for target domains with no cooperative extension point: targets are declared statically in the module config, `buildCompatInstrumentations` produces the load-time instrumentations (installed before the target module is imported), and `observe(name, listener)` registers the low-level patch as the calling fiber's effect. The public contract exposes only the declared target names, never `FabricPatch`, AST selectors, file paths, or `invoke()`. `observe` verifies the bridge installation capability (`isFabricInstalled`) and fails loud when the hooks are absent — resolving `ctx.fabric` alone is insufficient.
- **Client API.** Client commands and named UI slots through `ctx.command` and `ctx.slots`. The slot registration face is intentionally narrow (`FabricSlotOptions`): the full SlotMap type machinery lives in `@deepseek-ai/dsh-client-ui-slots`, whose declaration merging only sees the packages each consumer imports.
