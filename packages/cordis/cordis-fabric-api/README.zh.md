...
- **Compat API。** 面向没有协作式扩展点的目标 domain 的 patch-backed 缺口 adapter：目标在模块 config 中静态声明，`buildCompatInstrumentations` 生成加载期 instrumentations（在目标模块 import 前安装），`observe(name, listener)` 把低层 patch 注册为调用 fiber 的 effect。对外契约只暴露声明的目标名，绝不暴露 `FabricPatch`、AST selector、文件路径或 `invoke()`。`observe` 会验证 bridge installation capability（`isFabricInstalled`），hooks 缺失时失败即显式——仅能解析 `ctx.fabric` 不足以证明安装。
- **Client API。** 通过 `ctx.command` 和 `ctx.slots` 注册 client command 和命名 UI slot。Slot registration face 刻意保持窄（`FabricSlotOptions`）：完整的 SlotMap 类型机制位于 `@deepseek-ai/dsh-client-ui-slots`，其声明合并只能看到每个 consumer 导入的包。
