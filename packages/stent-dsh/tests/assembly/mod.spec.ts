...
async function assemble() {
  ...
  const fixtureUrl = new URL('../fixtures/node_modules/stent-api-fixture-mod/index.mjs', import.meta.url).href
...
describe('Stent API assembled composition', () => {
  ...
  it('boots an unmodified fixture Mod and observes all four effects through the assembled app', async () => {
    ...
    expect((globalThis as Record<string, unknown>).__stentApiFixtureSeen).toContain('assembly-agent:running')
  ...
  it('removes every contribution when the Mod fiber is disposed (HMR safety)', async () => {
    ...
    expect((globalThis as Record<string, unknown>).__stentApiFixtureSeen).toBeUndefined()
