...
describe('StentPromptService', () => {
  it('registers sections and variables through the authoritative registry', async () => {
    ...
    ctx.stentPrompt.section({ name: 'mod-identity', order: -50, text: 'identity: {{mod_name}}' })
  ...
  it('removes a section on its disposer', async () => {
    const ctx = await setup()
    const dispose = ctx.stentPrompt.section({ name: 'mod-ephemeral', order: 0, text: 'gone soon' })
  ...
  it('removes contributions when the contributing fiber disposes (HMR safety)', async () => {
    ...
    const mod = await ctx.plugin({
      ...
      apply(modCtx: Context) {
        modCtx.stentPrompt.section({ name: 'mod-scoped', order: 10, text: 'scoped' })
        modCtx.stentPrompt.context({ name: 'mod-cache', order: 1, text: 'cached' })
      },
    })
  ...
  it('inherits authoritative duplicate-name and invalid-name failures', async () => {
    ...
    expect(() => ctx.stentPrompt.section({ name: 'dup', order: 1, text: 'second' })).toThrow(/already registered/)
    expect(() => ctx.stentPrompt.variable('UPPER', () => 'x')).toThrow(/invalid prompt variable name/)
  })
})
